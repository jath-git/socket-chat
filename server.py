...
def server_menu():
    queue.push_simple(pad_dash('menu'))
    queue.push_simple("[OPTION] 1 - Kick a client")
    queue.push_simple("[OPTION] 2 - Disconnect all clients")
    queue.push_simple("[OPTION] 3 - Send a message to all clients")
    queue.push_simple("[OPTION] 4 - Shutdown server")
    queue.push_simple("[OPTION] 5 - Exit menu\n")
    ...
    if option == '1':
        ...
        queue.push_simple('[INPUT] Enter message to send:')
        ...
        queue.push('ERROR', 'Invalid Input. Choose from 1 to 5')
        queue.push_empty()
        server_menu()
...
def read_input():
    ...
    if len(user_input) > 0 and (user_input == '*' or user_input[len(user_input) - 1] == '*'):
        ...
        server_menu()
        queue.push_empty()
        queue.push_simple(pad_dash(' chat server '))
...
def boot_server():
    try:
        ...
        server.listen()
        clear_console()
        queue.push_simple(pad_dash('instructions'))
